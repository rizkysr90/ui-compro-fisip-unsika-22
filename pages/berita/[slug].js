...
export default function ProdukUnitUsaha({berita}) {
    if (!berita) {
        return (
            <p>Data not found</p>
        )
    }
    const {attributes : data} = berita
    return (
        <Layout>
                ...
                <div className="hero h-96 relative" style={{backgroundImage: `url(${data.cover_image.data.attributes.url})`}}>
                    ...
                    <div className="hero-content text-center absolute -mt-10  w-full text-base-100">
                        <div className="lg:w-2/4 w-full ">
                            ...
                            <div className="badge badge-secondary text-primary font-bold">{data?.kategori}</div>
...
export async function getServerSideProps({params}) {
    const {slug} = params;
    const beritaParam = qs.stringify({
        populate: ['cover_image'],
    }, 
    {
        encodeValuesOnly: true
    })
    const beritaData = await fetcher(
        `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/slugify/slugs/berita/${slug}?${beritaParam}`);
    return {
        props : {
